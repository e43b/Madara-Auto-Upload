...
if len(sys.argv) > 1:
    ...
    print("Nenhuma URL foi fornecida. Use o comando: python script.py <URL>")
    sys.exit(1)
...
artistas = [a.get_text() for a in soup.select('div.post-tags span:-soup-contains("Artista") ~ a')]
tags = [a.get_text() for a in soup.select('div.post-tags span:-soup-contains("Tags") ~ a')]

# Obtendo URLs das imagens (mantendo o formato original .webp)
image_urls = [img['src'] for img in soup.select('div.foto img')]
