...
def extrair_e_salvar_imagens(url, diretorio_atual, tipo_obra):
    ...
    if tipo_obra == 'manga':
        pasta_destino = os.path.join(diretorio_atual, f"Capítulo {capítulo}")
...
# URL do manga ou oneshot
url = "https://hiper.cool/manga/hatsujo-approach/"
...
# Criação inicial do JSON com informações básicas
info_data_initial = {
    "originalLink": url,
    "title": title,
    "synopsis": sinopse,
    "cover": "",  # Será preenchido após salvar a capa
    "pages": "",  # Será preenchido após criar o ZIP
    "oneshotTitle": "Oneshot" if tipo_obra == 'oneshot' else "",  # Define "Oneshot" para oneshot
    "type": tipo_obra,
